  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "https://localhost:7009/api/admin/add-consumable-item",
        {
          method: "POST",
          credentials: "include",
          headers: {
            ...
            <Col md={4} sm={6} xs={12} className="mb-3" key={key}>
              <Form.Group controlId={`${sectionName}-${key}`}>
                ...
                <Form.Control
                  ...
                  value={value}
                  onChange={(e) =>
                    handleNutritionalChange(sectionName, key, e.target.value)
                  }
                />
              </Form.Group>
            </Col>
    ...
    <Container className="add-consumable-form mt-5 mb-5">
      ...
      <Form onSubmit={handleSubmit}>
        ...
        <Accordion defaultActiveKey="0" className="mb-3">
          ...
          {renderNutritionalSection(
            "AminoAcids",
            nutritionalInfo.AminoAcids,
            "Amino Acids"
          )}
          {renderNutritionalSection("Fats", nutritionalInfo.Fats, "Fats")}
          {renderNutritionalSection(
            "Minerals",
            nutritionalInfo.Minerals,
            "Minerals"
          )}
          {renderNutritionalSection(
            "Sterols",
            nutritionalInfo.Sterols,
            "Sterols"
          )}
          {renderNutritionalSection(
            "More",
            nutritionalInfo.More,
