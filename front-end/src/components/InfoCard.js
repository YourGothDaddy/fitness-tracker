...
const InfoCard = ({ title, current, goal, unit }) => {
    ...
    return (
      <div className="info-card">
        ...
        <div className="weight-content">
          ...
          <div className="goal-status">
            <span className={isAboveGoal ? 'above-goal' : 'below-goal'}>
              {isAboveGoal ? 'Over goal by' : 'Under goal by'} {Math.abs(difference)} {unit}
            </span>
          </div>
        </div>
        <div className="progress-bar">
          <div 
            className="progress" 
            style={{ width: `${Math.min(percentage, 100)}%`, backgroundColor: getProgressColor() }}
          ></div>
        </div>
        <div className="motivational-text">
          {isAboveGoal ? "Keep going! You're almost there!" : "Great progress! Keep it up!"}
        </div>
      </div>
    );
  };
